...
namespace bnp {
	...
	void ResourceManager::add_mesh(Mesh mesh) {
		meshes.push_back(mesh);
	}

	void ResourceManager::add_material(Material material) {
	...
	void ResourceManager::cleanup_meshes(const entt::registry& registry) {
		...
		for (const Mesh& mesh : meshes) {
			if (std::find(used_va_ids.begin(), used_va_ids.end(), mesh.va_id) == used_va_ids.end()) {
				// if va isn't found
				unused_va_ids.push_back(mesh.va_id);
			}
			if (std::find(used_vb_ids.begin(), used_vb_ids.end(), mesh.vb_id) == used_vb_ids.end()) {
				// if vb isn't found
				unused_vb_ids.push_back(mesh.vb_id);
			}
			if (std::find(used_vb_ids.begin(), used_vb_ids.end(), mesh.eb_id) == used_vb_ids.end()) {
				// if eb isn't found
